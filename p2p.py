        ...
        while True:          # Loops forever
            ...
            for connection in self.clients_list:         # Send data to all clients in the list
                if ftype == 2:   # if the data received is an image file
                    d = connection.recv(BUFFER_SIZE).decode()
                    img = open(d, 'rb')
                    ...
                    img.close()
                    print("Image sent to", str(addr[0]) + ':' + str(addr[1]))
                   
                if ftype == 3:
                    connection.send(data.encode())        # except the one sending the data
...
class P2PClient:        # Class for the P2P client
    ...
    def __init__(self, address):     # Constructor
        ...
        while True:
            # print list of addresses connected to the server
            #print(P2PServer.clients_list)
            #dest = input ("Enter the destination IP address: ")     # Get the destination IP address
